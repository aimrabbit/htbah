...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'howtobeahero', 'HowToBeAHero', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.howtobeahero/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.howtobeahero/g),
        replacement: `flags.${this.propName}`
      },
      {
        pattern: 'howtobeahero',
   ...
   * Rename files.
   *
   * Rename files that had howtobeahero in their name, such as
   * css/howtobeahero.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*howtobeahero*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('howtobeahero', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-howtobeahero-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob HowToBeAHero's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace howtobeahero name mentions in files.
      generator.replaceFileContents();
      // Rename files that had howtobeahero in their name.
